...
import java.io.File
import uniffi.tinybot_mobile.pushOpen
...
/// where, and the first words. A tap opens the chat through the app's `tinybot://chat/<id>` link.
class PushService : FirebaseMessagingService() {
  override fun onMessageReceived(message: RemoteMessage) {
    ...
    val home = File(filesDir, "tinybot/core").absolutePath
    ...
    val open = Intent(Intent.ACTION_VIEW, Uri.parse(if (notice != null) "tinybot://chat/${notice.chatId}" else "tinybot://")).setPackage(packageName)
    ...
    val notification = NotificationCompat.Builder(this, CHANNEL)
      ...
      .build()
    manager.notify(notice?.chatId ?: "tinybot", (message.messageId ?: body).hashCode(), notification)
  }
