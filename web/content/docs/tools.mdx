...
Every command the chat's bots have running shows under **Running tasks**, including the ones a bot is still dealing with. While a chat has one, a terminal button with their number sits in the chat's toolbar, beside **⋯** on a phone. It opens the list: what each command does, who runs it and for how long, the command, its latest output, and **Stop**. **Stop** ends that command alone; the bot hears that it stopped and carries on. A command shows there once it has run for two seconds, so quick ones never do.
