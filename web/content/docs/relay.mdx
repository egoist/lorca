...
| `--quota-bytes` | Storage cap per identity. `0` for none. |
...
To send pushes to the phone app, give the relay an APNs key (`--apns-key`, `--apns-key-id`, `--apns-team-id`) or a Firebase service account (`--fcm-service-account`). Without them everything else works and phones get no pushes.
