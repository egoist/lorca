...
export function RelayDiagram() {
  return (
    <svg viewBox="0 0 700 360" className="w-full" role="img" aria-label="Three Macs exchange sealed envelopes through a relay that stores ciphertext only.">
      ...
      {machines.map((m) => (
        <g key={m.name} transform={`translate(${m.x - 70} ${m.y - 26})`}>
          <rect width="140" height="52" rx="12" fill="#18181b" stroke="rgba(255,255,255,0.14)" />
          <circle cx="20" cy="26" r="4" fill="#34d399" />
          <text x="34" y="23" fill="#f4f4f5" fontSize="13" fontWeight="600" fontFamily="Inter, system-ui">{m.name}</text>
      ...
      <g transform={`translate(${relay.x - 84} ${relay.y - 30})`}>
        ...
        <text x="84" y="26" textAnchor="middle" fill="#f4f4f5" fontSize="13" fontWeight="600" fontFamily="Inter, system-ui">Relay</text>
        <text x="84" y="44" textAnchor="middle" fill="#a1a1aa" fontSize="11" fontFamily="SF Mono, Menlo, monospace">a7f2…c9e1 · ciphertext</text>
      </g>
    </svg>
  )
}
