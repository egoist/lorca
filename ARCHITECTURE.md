...
6. **Provider credentials belong to the account.** API keys and ChatGPT and Grok tokens are connected once, on any Mac, and reach every paired Device as a `credentials` blob encrypted with the account DEK. A bot runs with them on whichever Runner it is assigned to.
...
- **Connecting** an API key checks it against the provider from the Mac it was typed on; a sign-in opens the browser on that Mac. **Disconnecting** removes the credential from every Device.
