...
6. **Provider credentials belong to the account.** DeepSeek and Anthropic keys and ChatGPT and Grok tokens are connected once, on any Mac, and reach every paired Device as a `credentials` blob encrypted with the account DEK. A bot runs with them on whichever Runner it is assigned to.
