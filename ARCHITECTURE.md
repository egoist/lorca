...
Shell review lives at the loop's `before_tool_call` boundary (`crates/cli/src/local_review.rs`). Every `bash` call is split into its `&&`, `||`, pipe, semicolon, and newline stages without executing it, described as read-only, effectful, networked, dynamic, or dangerous, and sent through Auto-review before the whole command starts. Built-in high-risk forms (destructive commands, privilege and access changes, opaque interpreters and wrappers, external execution channels, uploads, and force pushes) always ask unless the user already saved an exact rule. An Always allow rule is bound to the Runner, canonical working directory, and exact command arguments. A denial runs none of the stages.

**Auto-review** (`crates/cli/src/plugins/review.rs`) is shared by shell and plugin actions. Its setting lives in the roster (`AutoReview { is_enabled, rules[{ id, text, behavior: allow | ask, tool?, runner_id?, workdir?, command? }] }`), so every Device shows and edits the same one. With Auto-review on, a card-created rule carries an exact action key and decides at once; a local shell rule also records its Runner, working directory, and complete command so Settings shows exactly what was allowed and a deleted private workspace can shed rules that no longer apply. User-written natural-language rules are considered only for the current action. Without an exact rule, the bot's own model (thinking off, 200 tokens) judges the action against those rules, the built-in checks, and the user's latest message, answering `allow` or `ask` with a reason; an ask rule wins over an allow rule. With Auto-review off, every shell command and effectful plugin action asks, regardless of saved rules. A held action posts a `permission` message and waits up to ten minutes for `chats.permission { decision: allow | always | deny }` from any Device. A routine has nobody waiting, so an action that would ask is refused; with Auto-review on, automatically approved actions and exact allow rules still run.
