...
**At turn time** (`crates/cli/src/plugins/mcp.rs`, `rmcp`), the Runner connects each installed plugin's servers on first use and keeps them in a pool (stdio children, or streamable HTTP with the plugin's headers, a pasted token, or the saved OAuth tokens), lists their tools, and offers them to the model as `<plugin>__<tool>` with the server's instructions and the plugin's skills in the system prompt. Browser-flow OAuth tokens are refreshed by the transport and written back as soon as the handshake rotates them. A device-flow token is a plain bearer; the Runner refreshes and saves its rotating pair itself, and drops the pooled connection before that bearer expires so the next turn reconnects with a fresh one. A tool the server marks read-only (`readOnlyHint`) or the manifest lists as such runs at once. Any other tool goes through the same Auto-review boundary as local actions. When it asks, the card reads "Chef wants to use GitHub · create_issue · repo: …"; Always allow saves the exact `plugin/tool`, and the working row reads "Using GitHub…" while the call runs.
