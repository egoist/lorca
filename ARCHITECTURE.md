...
`file` ciphertext can live outside the database (`store.rs`): `--files-dir` keeps it in a directory, `--s3-bucket` with `--s3-endpoint` (and the access keys, `TINYBOT_RELAY_S3_*` or `AWS_*`) in an S3-compatible bucket (AWS, R2, MinIO) with SigV4 and path-style URLs. Objects are keyed `<identity pubkey>/<blob id>`; the row keeps the metadata with `external = 1` and an empty `ciphertext`. The object goes up before the row and is removed when the row is refused or deleted. With neither flag, files stay in SQLite, the self-host default.
