...
Chrome: split view, vibrancy, bubbles, `@` mentions. The window's titlebar is transparent and the content pane draws the titlebar material and its separator itself (`ContentContainerViewController`), so the strip's edge is the pane's edge; pages other than the transcript keep their scroll view below the safe area. The app renders CLI events and applies its own edits optimistically; `LORCA_MOCK=1` runs the seeded demo instead. Keys stay in the CLI.
