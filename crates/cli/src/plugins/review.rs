//! Auto-review: the check a Runner runs before a plugin tool that changes something, after
//! Grok Bot's. An exact rule (from a card's Always allow) decides at once; otherwise, with
...
const SYSTEM_PROMPT: &str = "You are Auto-review, the safety check that runs before a bot acts on a connected service. \
...
has access, affects many items at once, or cannot be undone easily. Allow contained, reversible work in the user's own \
space that the user's request plainly calls for: creating a draft, an issue, a page, a branch, or a task; editing or \
closing something the bot itself just made; updating a field the user asked to change. The user's rules come first: \
...
/// Decides one plugin action for `bot`.
...
pub async fn decide(app: &Arc<App>, bot: &Bot, chat_id: &str, plugin_id: &str, plugin_name: &str, tool: &str, description: &str, args: &Value, cancel: &CancellationToken) -> Outcome {
    ...
    if let Some(rule) = auto_review.rule_for(plugin_id, tool) {
    ...
    let allow: Vec<&str> = auto_review.rules.iter().filter(|r| r.behavior == "allow").map(|r| r.text.as_str()).collect();
    let ask: Vec<&str> = auto_review.rules.iter().filter(|r| r.behavior == "ask").map(|r| r.text.as_str()).collect();
    ...
    text.push_str(&format!(
        "The action: bot {} wants to call {tool} on {plugin_name}.\nWhat the tool does: {}\nArguments:\n{arguments}",
        bot.name,
